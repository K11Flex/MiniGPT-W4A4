...
本项目演示了如何对 MiniGPT 模型进行 **W4A4 量化**（权重 4-bit、激活 4-bit）并使用 NumPy 做前向推理和文本生成。流程基于 SmoothQuant + AWQ 分组量化。
...
│  ├─ model_w4_sym_g8.npz     # W4 权重量化后的权重
│  └─ scales_w4_sym_g8.npz    # 每组权重的 scale
├─ smoothquant_calibration.py   # SmoothQuant 缩放与激活校准
├─ quantize_weights.py          # 权重量化（G8 + MSE 最小化）
...
- 输出：
  - `weights/model_w4_sym_g8.npz`
  - `weights/scales_w4_sym_g8.npz`
- 功能：按组对权重进行 4-bit 对称量化
